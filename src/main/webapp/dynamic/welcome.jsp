<%@page import="id.prasetya.vibrefy.tools.Escape,id.prasetya.vibrefy.beans.SetupBean,id.prasetya.vibrefy.beans.PairBean"%><jsp:useBean id="bean" scope="request" class="id.prasetya.vibrefy.beans.BeanObject" /><?xml version="1.0" ?>
...
<button id="pairbtn">🔗 Sign in from another device</button>
</div>
<%
} else
{
...
// For devices that cannot practically run the sign-in popup - a TV box, a handheld, a
// headset. The pairing page carries a login over from a device that is already signed in.
var pairbtn=document.getElementById('pairbtn');
if (pairbtn)pairbtn.addEventListener('click',function()
{
 makeRequest('<%=PairBean.CMDPair%>','',true);
},false);
<%
} else
{
